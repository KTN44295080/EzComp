...
export function imageStats(imageData: ImageData): ImageStats {
  ...
  let pixels = 0, sumLuminance = 0, sumLuminanceSquared = 0, sumSaturation = 0, sumR = 0, sumG = 0, sumB = 0;
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    pixels += alpha; sumLuminance += luminance * alpha; sumLuminanceSquared += luminance * luminance * alpha; sumSaturation += saturation * alpha; sumR += r * alpha; sumG += g * alpha; sumB += b * alpha;
  ...
  const luminance = sumLuminance / pixels, channelR = sumR / pixels, channelG = sumG / pixels, channelB = sumB / pixels;
...
function referenceStats(documentModel: CompositeDocument, layers: RasterLayer[], selected: RasterLayer, referenceId: string): ImageStats {
...
export function autoCompositeLayer(documentModel: CompositeDocument, layers: RasterLayer[], selected: RasterLayer, preset: AutoCompositePreset, referenceId = 'below'): LayerAdjustments {
  if (selected.kind === 'group') throw new Error('Choose a raster layer to auto composite.');
  const adjustments = deriveAutoAdjustments(statsForLayer(selected), referenceStats(documentModel, layers, selected, referenceId), preset);
